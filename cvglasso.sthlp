...
{synopt:{opt crit(string)}} Cross validation criterion ({it: loglik}, and {it: extended BIC}). Default is {it: loglik} {p_end}
...
{phang2}{cmd:. cvglasso data, nfold(5) nlam(40) crit(loglik)}{p_end}
...
{pstd} Estimate sparse inverse covariance matrix based on extended BIC.{p_end}
...
{pstd}
{cmd:CVglasso} stores the following in {cmd:r()}:

{synoptset 20 tabbed}{...}
{p2col 5 20 24 2: Scalar}{p_end}
{synopt:{cmd:r(lambda)}}Tuned regularization parameter.{p_end}
{p2colreset}{...}
